...
export class FaceFactory {
    ...
    static createFaces(position: CubePosition, dimension: number): Array<Face> {
        ...
        if(position.i == 0) faces.push(new Face(LeftSide.get()))
        if(position.j == 0) faces.push(new Face(DownSide.get()))
        if(position.k == 0) faces.push(new Face(BackSide.get()))
        if(position.i == dimension - 1) faces.push(new Face(RightSide.get()))
        if(position.j == dimension - 1) faces.push(new Face(UpSide.get()))
        if(position.k == dimension - 1) faces.push(new Face(FrontSide.get()))
        return faces
    }
}
